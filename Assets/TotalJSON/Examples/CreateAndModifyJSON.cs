...
namespace Leguar.TotalJSON.Examples {
	...
	public class CreateAndModifyJSON : MonoBehaviour {
		...
		private void Test1() {
			...
			Log("Note! You can see the content of selected JSON objects while application is running by choosing \"Window -> Total JSON -> JSON Runtime Debug\" from Unity menu");
			...
			// Print out
			Log(jsonObject.CreateString()); // {"text":"Hello World!","number":42,"truth":true,"nullText":null,"numberArray":[1,2,4,8]}
			// Colored print out, just for debug purposes
			Log(jsonObject.CreateString(new CreateStringSettings() { ColoredOutput = true }));
			...
			Log("Info of all the values:");
			foreach (string key in jsonObject.Keys) {
				Log(key + ": " + jsonObject[key]);
			}

			// Loop through all the keys and print out all values in JSON formatted strings
			Log("Content of all the values:");
			foreach (string key in jsonObject.Keys) {
				Log(key + " -> " + jsonObject[key].CreateString());
			}

			// Get and print out some values
			Log("Print outs of some values:");
			Log(jsonObject.GetString("text")); // "Hello World!"
			Log(jsonObject.ContainsKey("number")); // False
			Log(jsonObject.GetBool("truth")); // False
			Log(jsonObject.GetString("nullText")); // (null)
			...
			Log("Array length: " + jsonArray.Length); // 5
			Log("Fifth element as int: " + jsonArray.GetInt(4)); // 16
			...
			// Print out values
			Log("Second element as int: " + systemIntArray[1]); // 4
		...
		private static void Log(object str) {
			Debug.Log("[CreateAndModifyJSON] " + str);
		}
